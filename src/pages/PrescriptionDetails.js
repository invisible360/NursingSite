...
import useAppState from '../hook/hook'; //Hook
...
const PrescriptionDetails = () => {
    ...
    const { data: prescribedMedicines = [], isLoading, refetch } = useQuery({
        queryKey: ['prescriptionList'],
        queryFn: async () => {
            const res = await fetch(`http://hospital.rushdi.pw/api/prescription/list`);
    ...
    const handleHistory = (med) => {
        const dosesInNumber = parseInt(med.doses);
        const updateDoses = dosesInNumber - 1;
        //**time-slot should not be required field for updating presciption list while disposed_time need to add & update in history list **//

        //updating prescription doses 
        fetch(`http://hospital.rushdi.pw/api/prescription/update/${med.id}?time_slot= ${format(new Date(), "HH:mm:ss")}&medicine=${med.medicine}&doses=${updateDoses}`, {
            method: "PUT",
            headers: {
            ...
            .then(result => {
                refetch();
                if (result.success) {
                    //searching medicine from hisory list
                    fetch(`http://hospital.rushdi.pw/api/history/edit/${med.id}`)
                        .then(res => res.json())
                        .then(data => {
                            const quantityInNumber = parseInt(data.quantity);
                            const updateQuantity = quantityInNumber + 1;
                            const dosesInNumber = parseInt(data.doses);
                            const updateDoses = dosesInNumber - 1;
                            const medicine = data.medicine;
                            const nurse_id = data.user_id;
                            const CurrMedId = data.id;

                            //updating quantity in history table
                            fetch(`http://hospital.rushdi.pw/api/history/update/${CurrMedId}?medicine=${medicine}&quantity=${updateQuantity}&user_id=${nurse_id}&doses=${updateDoses}`, {
                                method: "PUT",
                                headers: {
                                    "content-type": "application/json"
                                }
                                ...
                                .then(result => {
                                    if (result.success) {
                                        // refetching updated histroy table
                                        fetch(`http://hospital.rushdi.pw/api/history/list`)
                                            .then(res => res.json())
                                            .then(data => {
                                                setHistoryData(data);
                                            })
                                                        ...
                                                        <div className="row g-0">
                                                            ...
                                                            <div className="col-md-9 card-body">
                                                                <ol className="list-group list-group-numbered">
                                                                    {
                                                                        prescribedMedicines.map((m, Mi) =>
                                                                            <li key={Mi} type="button" onClick={() => {
                                                                                handleHistory(m);
                                                                            ...
                                                                            >
                                                                                <div className="ms-2 me-auto">
                                                                                    ...
                                                                                    <div className="fw-bold">
                                                                                        ...
                                                                                        <span className={`${m.doses === 0 && 'text-danger text-decoration-line-through'}`}>{m.medicine}</span>
                                                                                    </div>
                                                                                </div>
